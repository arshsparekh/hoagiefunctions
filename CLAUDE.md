...
| `/event/:id` | Event detail: map, attendees, capacity bar, participation actions (rsvp / waitlist / applyToEvent / withdraw), save/share/`.ics`, and a **Discussion** thread. The guestlist **auto-accept** success banner is the key demo beat - keep it prominent. Managers get door check-in (+ CSV export), "Manage guestlist", and cancel-event. |
...
- **Notifications** are a flat, persisted list generated by actions (approvals,
  waitlist promotion, new comments, new events from followed hosts, reschedules)
  plus a small data-derived seed. Use `myNotifications()`/`unreadCount()`/
  `markAllNotificationsRead`.
- **Comments** are a flat, persisted `comments` list (one row per event comment).
  `addComment` (any viewer who can see the event; sanitized + length-bounded,
  notifies the host) and `deleteComment` (author or event manager) guard writes;
  `commentsForEvent(id)` reads them. `deleteEvent` cleans up an event's comments.
